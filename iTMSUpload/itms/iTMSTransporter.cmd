...
set JVM=.\java\bin\java.exe
set JVMOPTS=-Xmx2048m -Xms1024m -Djava.awt.headless=true -Dsun.net.http.retryPost=false -Dhttps.protocols=TLSv1.1,TLSv1.2
set APPLICATION_CLASS=com.apple.transporter.launcher.Application
...
:: Set up the java module options, it is safe to do this now because we include JDK-11 as part of the install package.
:: So we always know which version of java is running.
:: The current configuration is for OpenJDK-11.
set JAVA_MODULES=%JAVA_MODULES% --illegal-access=deny
set JAVA_MODULES=%JAVA_MODULES% -p .\modules
set JAVA_MODULES=%JAVA_MODULES% --add-modules=java.activation,java.net.http,java.sql,java.xml,java.xml.bind
set JAVA_MODULES=%JAVA_MODULES% --add-exports=java.base/sun.security.provider.certpath=ALL-UNNAMED
set JAVA_MODULES=%JAVA_MODULES% --add-opens=java.xml/com.sun.org.apache.xalan.internal.xsltc.trax=ALL-UNNAMED
...
:: echo Launching %~n0.
:: echo "Launching iTMSTransporter with: %JAVAEXE% %JVMOPTS% %JAVA_MODULES% %JAVAARGS% -jar .\lib\itmstransporter-launcher.jar %THEARGS%"
call %JAVAEXE% %JVMOPTS% %JAVA_MODULES% %JAVAARGS% -jar .\lib\itmstransporter-launcher.jar %THEARGS%
